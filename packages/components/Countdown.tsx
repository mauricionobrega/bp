...
function CountdownComponent(props: any) {
  const getCountdown = () => {
    ...
    const timeRemaining = new Date(`${year}-1-1`).getTime() - new Date().getTime();
    let countdown = {
      dias: 0,
      horas: 0,
      min: 0,
  ...
  return (
    <div className={styles.countdown}>
      {countdown ? Object.entries(countdown).map(([unit, value]) => {
        return <div key={unit} className={styles.wrapper}>
