  ...
  return (
      <div className={styles.plans}>
        <div className={styles.wrapperTitles}>
          ...
          <h2 className={styles.plansHeadingTitle}>Uma Brasil Paralelo para cada momento.</h2>
          <p className={styles.plansParagraph}>Você pode assistir no computador ou em qualquer aparelho conectado à internet, como Smart TVs, smartphones e tablets.</p>
        </div>
        ...
        <Row justify="space-evenly">
          {PlansItems.map(item => {
            ...
            return <Col span={7} className={styles.shelf} key={text}>
              <div className={styles.shelfWrapper}>
                <span className={styles.shelfHeading}>{text}</span>
                {badge ? <span className={styles[badgeClassName]}>{badgeText}</span> : ''}
              </div>
              <div className={styles.shelfWrapper}>
              ...
              <span className={styles.shelfPackageContents}>{packageContents}</span>
              <div className={styles.shelfWrapper}>
                <Link href={href} passHref>
                  <a target="_blank" className={styles.shelfButtonText}>{buttonText}</a>
                </Link>
              </div>
              ...
              <div className={styles.shelfFeaturesWrapper}>
                <ul className={styles.shelfFeatures}>
                  {features.map(feature => {
                    return <li className={styles.shelfFeaturesItem} key={feature}>
                      <Image
                        ...
                        src={FeatureBullet}
                      />
                      <span className={styles.shelfFeatureItemText}>{feature}</span>
                    </li>
                  })}
                </ul>
              </div>
